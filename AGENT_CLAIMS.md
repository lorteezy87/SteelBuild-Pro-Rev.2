...
| 2026-07-05T00:00:00Z | opus-detailing-sp5-viewer | Detailing CC re-skin SP5 — Viewer + 3D chrome (flag `command_ui`, `.detailing-cc` lever, NO command.css change) | src/pages/DrawingViewer.jsx, src/pages/drawingViewer/drawingViewerStyles.js, src/components/viewer3d/Model3DTab.jsx | Re-skin the DrawingViewer surrounding chrome (header/toolbar/sidebar/hints/empty-states) + the 3D Model tab chrome onto the light kit under command_ui; canvas viewport + floating canvas-overlay markup/zone toolbars stay dark (out of scope → SP5-2). Flag-off byte-identical. |
