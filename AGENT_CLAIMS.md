...
| 2026-07-05T00:00:00Z | detailing-cc-sp3 | Detailing CC native re-skin — SP3 Boards & Matrix (flag `command_ui`, presentation-only, does NOT touch `src/styles/command.css` — respects opus-detailing-panel-light). New `*.derive.ts` + `*Panel.tsx`, swapped `commandUi ? <Panel/> : <Existing/>` at hub `activeTabPanel` (byte-identical flag-off). Working-day due display (`submittal_workday_dues`) preserved verbatim. | src/components/submittals/processBoard.derive.ts, src/components/submittals/ProcessBoardPanel.tsx, src/pages/drawingSubmittalHub/{ApprovalMatrixPanel,RevisionImpactPanel}.tsx, src/pages/DrawingSubmittalHub.tsx (3 swap points only) | Convert Process Board + Approval Matrix + Revision Impact board onto the kit. StagePipeline chevrons deferred (lives on Drawings.jsx, outside the detailing command island). |
