...
| 2026-07-05T12:00:00Z | detailing-cc-slice2a-drawing-register | Detailing Control Center re-skin — Slice 2a: convert the Drawing Register (`drawings` tab) onto command kit primitives (FilterBar + kit table / re-skinned virtual grid), INSIDE the shipped shell, gated by `command_ui`. **Presentation-only, behavior-preserving** (same columns/rows/filters/chips/row-actions/modals; `["drawing-register", projectId]` cache key + all mutations unchanged). Does NOT touch `src/styles/command.css` (uses existing `cmd-*` + shared cell renderers) — respects opus-detailing-panel-light + opus-command-ui-lock. | src/pages/drawingSubmittalHub/DrawingRegisterPanel.tsx (NEW), src/pages/drawingSubmittalHub/drawingRegister.derive.ts (NEW) + its `__tests__`, src/pages/DrawingSubmittalHub.tsx (swap `drawings` tab `DrawingRegisterTable`→`DrawingRegisterPanel` under commandUi only) | Slice 2a of the sliced native conversion (Submittal Register=2b, Doc Control=2c split out). |
