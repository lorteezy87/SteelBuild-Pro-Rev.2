...
| 2026-07-05T20:30:00Z | opus-detailing-cc-slice2b | Detailing command_ui re-skin — Slice 2b: Submittal Register. **Light-aligned with opus-command-ui-lock + detailing-panel-light** (command_ui light skin; does NOT touch `src/components/command/**` or `src/styles/command.css`). Presentation-only, flag-gated, behavior-preserving (splitting/lineage + per-type + working-day + revision all reused as-is). | src/pages/Submittals.tsx, src/pages/submittals/SubmittalRegisterPanel.tsx (new), src/pages/submittals/submittalRegister.derive.ts (new) + its __tests__, src/pages/DrawingSubmittalHub.tsx (submittals-tab swap point only) | Convert the Submittal Register chrome (header/KPI strip/filter bar) onto kit primitives under command_ui; reuse SubmittalVirtualList + SubmittalDetail verbatim. `commandUi ? <SubmittalRegisterPanel/> : <legacy chrome>` inside Submittals.tsx. Flag-off byte-identical. |
