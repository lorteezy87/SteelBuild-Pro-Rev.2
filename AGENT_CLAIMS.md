...
| 2026-07-05T00:00:00Z | detailing-cc-slice1-controlboard | Detailing Control Center re-skin — Slice 1: convert the Control Board (overview) tab content onto command kit primitives, INSIDE the already-shipped `DetailingCommandShell` (no parallel shell). **Presentation-only, behavior-preserving.** Does NOT touch `src/styles/command.css` (uses existing `cmd-*` classes + inline grids) — respects `opus-detailing-panel-light` + `opus-command-ui-lock`. | src/pages/drawingSubmittalHub/ControlBoardPanel.tsx (NEW), src/pages/drawingSubmittalHub/drawingControlCenter.derive.ts (NEW) + its `__tests__`, src/pages/drawingSubmittalHub/triageBoard.tsx (ADD `export` to reused sub-components only), src/pages/DrawingSubmittalHub.tsx (swap overview panel `TriageBoard`→`ControlBoardPanel` in the command branch) | Slice 1 of the sliced native conversion (owner-approved 2026-07-05). |
