...
| 2026-07-05T00:00:00Z | detailing-cc-sp4-modals | SP4 — Detailing CC modals → light dialog skin (flag-gated) | src/pages/drawingSubmittalHub/{leadTimesModal,EscalateModal}.tsx · src/components/drawings/{RevisionCompareModal,RevisionSummaryCard,ModelElementImportModal}.jsx + drawing upload/revision/markup/approval modals reachable from detailing tabs · src/components/rfis/RFIFormModal.jsx | Wrap each detailing-context modal's content root in the existing `.detailing-cc` scope (reuses shipped token-alias + `sbd-*`/`desk-*` light chrome), gated on `command_ui` so flag-off is byte-identical. **Does NOT touch `src/styles/command.css`** — pure wrapper approach, same as SP2b/2c. Light-aligned with `opus-command-ui-lock`. |
