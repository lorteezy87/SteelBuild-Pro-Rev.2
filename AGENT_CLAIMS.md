...
|---|---|---|---|---|
| 2026-09-23T13:48:43Z | codex-mob4-consolidation | MOB-4 native sign-in and dependency security carryover | src/pages/Landing.jsx; src/pages/__tests__/Landing*.test.*; src/boot/{AppProviders.jsx,dev/FoundationPreview.tsx}; src/__tests__/components/DrawingSubmittalHub.test.jsx; package.json; package-lock.json | Ship a clean native sign-in-only flow from current main and preserve the audited React Router/uuid fixes from the retiring desktop copy. |
