...
export const IMPORT_TARGETS = {
  ...
  sovItems: {
    ...
    entityKey: "SOVItem",
    required: ["description"],
    defaults: { status: "Open", current_percent_complete: 0, retainage_percent: 10 },
    allowedValues: {
      status: ["Open", "Submitted", "Approved", "Paid", "Closed"],
    },
    fields: [
      "line_item_number", "sov_id", "description", "scheduled_value",
      "previous_percent_complete", "current_percent_complete", "retainage_percent",
      "status", "application_number", "period_from", "period_to",
      "submitted_date", "payment_received_date",
    ],
    aliases: {
      line_item_number: ["line", "line item", "item number", "item #", "line #"],
      sov_id: ["sov id", "sov number", "sov #"],
      description: ["description", "item", "work description", "scope"],
      scheduled_value: ["scheduled value", "contract value", "value", "amount", "original value"],
      current_percent_complete: ["current %", "percent complete", "% complete", "pct complete"],
      previous_percent_complete: ["previous %", "prior %", "prev percent"],
      retainage_percent: ["retainage", "retainage %", "ret %"],
      application_number: ["application", "app #", "pay app", "draw #"],
      submitted_date: ["submitted", "submitted date"],
      payment_received_date: ["paid date", "payment date", "received date"],
    },
  },
  costCodes: {
    label: "Budget (Cost Codes)",
    entityKey: "CostCode",
    required: ["cost_code_number", "description"],
    defaults: { budget_amount: 0, actual_cost: 0, committed_cost: 0, forecast_to_complete: 0 },
    fields: [
      "cost_code_number", "description", "category", "phase",
      "budget_amount", "actual_cost", "committed_cost", "forecast_to_complete", "notes",
    ],
    aliases: {
      cost_code_number: ["code", "cost code", "code #", "number", "cc #"],
      description: ["description", "name", "line item"],
      category: ["category", "type", "group"],
      budget_amount: ["budget", "budgeted", "original budget"],
      actual_cost: ["actual", "actuals", "actual cost", "spent"],
      committed_cost: ["committed", "committed cost", "encumbered"],
      forecast_to_complete: ["forecast", "etc", "estimate to complete", "ftc"],
    },
  },
  expenses: {
    label: "Expenses",
    entityKey: "Expense",
    required: ["description"],
    defaults: { payment_status: "Pending", amount: 0 },
    allowedValues: {
      payment_status: ["Pending", "Approved", "Paid", "Rejected", "On Hold"],
    },
    fields: [
      "expense_number", "description", "expense_type", "cost_code", "cost_code_name",
      "amount", "quantity", "unit_cost", "unit", "vendor",
      "invoice_number", "invoice_date", "expense_date",
      "payment_status", "payment_date", "submitted_by", "approved_by", "notes",
    ],
    aliases: {
      expense_number: ["expense #", "exp #", "number"],
      description: ["description", "item", "detail"],
      expense_type: ["type", "expense type", "category"],
      cost_code: ["cost code", "code", "cc"],
      cost_code_name: ["cost code name", "code name"],
      amount: ["amount", "total", "cost"],
      vendor: ["vendor", "supplier", "payee"],
      invoice_number: ["invoice", "invoice #", "inv #"],
