...
import { APPROVAL_CHAIN_MONO, STEP_CHIP_PALETTE } from "@/components/submittals/approvalChainPanelHelpers";
import { APPROVAL_TEMPLATE_MONO, buildPartySuggestions } from "@/components/submittals/approvalChainTemplatesHelpers";
...
describe("residual catalog atoms batch G", () => {
  ...
  it("submittals/drawings mono tokens", () => {
    ...
    expect(STEP_CHIP_PALETTE.current.bg).toBe("var(--accent)");
    expect(buildPartySuggestions(["Owner"])).toEqual(
      expect.arrayContaining(["Owner", "PM", "Fabricator"]),
    );
  });
