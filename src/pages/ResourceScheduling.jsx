...
import { AlertTriangle, Clock3, Plus, UserCheck, Users } from "lucide-react";
...
function ResourceGuruCommandStrip({ plan, focus, onFocusChange }) {
  const focusOptions = [
...
function GuruMetric({ icon: Icon, label, value, sub, color }) {
  return (
...
export default function ResourceScheduling() {
  ...
  const onBoardPointerMove = (e) => {
    ...
    boardRef.current?.querySelectorAll("[data-resource-id]").forEach((row) => {
      ...
      const resCapacity = Number(effectiveCapacityById[resId] ?? res?.capacity ?? res?.budget_hours) || 0;
