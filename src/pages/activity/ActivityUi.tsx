/**
 * Presentational pieces for Activity page shell.
 */
// @ts-nocheck
...
import { ACTIVITY_DATE_RANGES } from "./activityPageHelpers";

export const activitySelectTriggerClass =
...
export function ActivityPageHeader({ hasActiveFilters, onClearFilters }) {
  return (
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
      <div>
        ...
        <Select value={filterProject} onValueChange={onFilterProject}>
          <SelectTrigger style={{ borderRadius: 8 }} className={activitySelectTriggerClass}>
      ...
      <div>
        ...
        <Select value={filterEntity} onValueChange={onFilterEntity}>
          <SelectTrigger style={{ borderRadius: 8 }} className={activitySelectTriggerClass}>
      ...
      <div style={{ display: "flex", alignItems: "flex-end" }}>
        <button
          ...
          style={{
            width: "100%",
            padding: "8px 12px",
            background: filteredCount > 0 ? "var(--accent-glow)" : "var(--bg-surface)",
            border: "1px solid var(--accent-border)",
            borderRadius: 8,
            color: filteredCount > 0 ? "var(--accent)" : "var(--text-muted)",
            fontFamily: "var(--font-mono)",
            fontSize: 10,
            fontWeight: 600,
            cursor: filteredCount > 0 ? "pointer" : "not-allowed",
            transition: "all 0.15s",
            opacity: filteredCount > 0 ? 1 : 0.5,
          }}
          onMouseEnter={(e) => { if (filteredCount > 0) { e.currentTarget.style.background = "var(--accent-hover)"; e.currentTarget.style.opacity = "0.85"; } }}
          onMouseLeave={(e) => { if (filteredCount > 0) { e.currentTarget.style.background = "var(--accent-glow)"; e.currentTarget.style.opacity = "1"; } }}
        >
          EXPORT CSV
        </button>
      </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
      <div style={{ background: "var(--hover-bg)", borderRadius: 10, padding: 14, border: "1px solid var(--divider)" }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 700, color: "var(--accent)", marginTop: 6 }}>
          {activitiesLoading ? "—" : filteredCount}
        </div>
      ...
      <div style={{ background: "var(--hover-bg)", borderRadius: 10, padding: 14, border: "1px solid var(--divider)" }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 700, color: "var(--status-success)", marginTop: 6 }}>
          {activitiesLoading ? "—" : uniqueUserCount}
        </div>
      </div>
      ...
      <div style={{ background: "var(--hover-bg)", borderRadius: 10, padding: 14, border: "1px solid var(--divider)" }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 700, color: "var(--status-info)", marginTop: 6 }}>
          {activitiesLoading ? "—" : uniqueEntityCount}
        </div>
      </div>
    </div>
  );
...
export function ActivityEmptyState({ onNavigateRfis, onNavigateDocuments, onNavigateWorkPackages }) {
  return (
    <div style={{
      padding: "48px 24px",
      textAlign: "center",
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center", marginTop: 8 }}>
        ...
        <button
          onClick={onNavigateWorkPackages}
          style={{
            padding: "8px 16px",
            background: "transparent",
            border: "1px solid var(--border-default)",
...
export function ActivityFilteredEmptyState({ onClearFilters }) {
  return (
    <div style={{
      padding: "40px 24px",
      textAlign: "center",
  ...
  return (
    <div style={{ background: "var(--hover-bg)", borderRadius: 12, padding: 16, border: "1px solid var(--divider)" }}>
      {activitiesLoading ? (
        ...
        <ActivityEmptyState
          onNavigateRfis={onNavigateRfis}
          onNavigateDocuments={onNavigateDocuments}
          onNavigateWorkPackages={onNavigateWorkPackages}
        />
      ) : filtered.length === 0 && hasActiveFilters ? (
        <ActivityFilteredEmptyState onClearFilters={onClearFilters} />
      ) : (
