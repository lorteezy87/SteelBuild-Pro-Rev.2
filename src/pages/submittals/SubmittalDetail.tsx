...
import type { ApproverNote } from "@/lib/approverNotes";
...
export function SubmittalDetail({
  ...
  const relatedSetRfis = useMemo(
    () => deriveRelatedSetRfis(submittal, allRfis),
    [submittal, allRfis],
  );
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 20px" }}>
        ...
        {drawingTypesEnabled && submittal.id && submittal.project_id && (
          <DetailSection title="Drawing types (Shop / Erection / Part)">
            <SubmittalDrawingTypeComponents
        ...
        <DetailSection title={`Approval Cycles (${rounds.length})`}>
          ...
          {onNewRound && canStartNewRound(submittal.status) && (() => {
            const newRoundAction = deriveNewRoundAction(submittal, rounds);
            return (
              <div style={{ marginTop: 8 }}>
                <button
                  ...
                  style={{
                    padding: "6px 14px",
                    borderRadius: 4,
                    background: newRoundAction.isResubmit ? "var(--status-review)" : "var(--accent)",
                    color: "var(--on-accent)",
                    border: "none",
                    fontFamily: "var(--font-mono)",
                    fontSize: 9,
