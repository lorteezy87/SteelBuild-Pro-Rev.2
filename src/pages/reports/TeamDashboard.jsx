...
const isOpenRFI = (r) => r.status !== "Closed" && r.status !== "Answered";
const isOpenAction = (a) => a.status !== "Complete" && a.status !== "Cancelled";
...
export default function TeamDashboard() {
  ...
  const people = useMemo(() => {
    ...
    rfis.filter(isOpenRFI).forEach((r) => {
      const a = (r.assigned_to || r.ball_in_court || "").trim();
      if (a) ensure(a).rfis += 1;
    });
    actionItems.filter(isOpenAction).forEach((it) => {
  ...
  const totalOpenRFIs = rfis.filter(isOpenRFI).length;
  const totalOpenActions = actionItems.filter(isOpenAction).length;
