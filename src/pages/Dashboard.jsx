...
export default function Dashboard() {
  ...
  const { data: projects = [], isLoading: projectsLoading } = useQuery({ queryKey: ["projects"], queryFn: () => base44.entities.Project.list(), staleTime: 5 * 60 * 1000 });
  const { data: allRFIs = [], isLoading: rfisLoading } = useQuery({ queryKey: ["rfis"], queryFn: () => base44.entities.RFI.list() });
  const { data: allCOs = [] } = useQuery({ queryKey: ["cos-all"], queryFn: () => base44.entities.ChangeOrder.list() });
  const { data: allCodes = [] } = useQuery({ queryKey: ["codes-all"], queryFn: () => base44.entities.CostCode.list() });
  const { data: allWPs = [] } = useQuery({
    queryKey: ["work-packages"],
    queryFn: () => base44.entities.WorkPackage.list(),
    staleTime: 30000,
  });
  const { data: allDeliveries = [] } = useQuery({ queryKey: ["deliveries-all"], queryFn: () => base44.entities.Delivery.list() });
  const { data: allActionItems = [] } = useQuery({ queryKey: ["action-items-all"], queryFn: () => base44.entities.ActionItem.list() });
  const { data: allExpenses = [] } = useQuery({ queryKey: ["expenses-all"], queryFn: () => base44.entities.Expense.list() });
  ...
  const { data: tasks = [] } = useQuery({ queryKey: ["schedule-tasks", pid], queryFn: () => pid ? base44.entities.ScheduleTask.filter({ project_id: pid }, "-start_date") : [], enabled: !!pid });
