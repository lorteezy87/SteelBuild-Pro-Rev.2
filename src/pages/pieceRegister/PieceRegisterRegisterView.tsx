...
import { nextPieceRegisterSort, type PieceRegisterSort } from "@/lib/pieceControl/pieceRegisterSort";
...
export type PieceRegisterRowView = {
  id: string;
  piece_mark: string;
  lot_code: string;
...
export type PieceRegisterRegisterViewProps = {
  ...
  filteredRows: PieceRegisterRowView[];
  displayRows: PieceRegisterRowView[];
        ...
        <div className="piece-register-table">
          ...
          {selectedPieceId ? (
            <DecisionPanel title="Piece impact">
              <PieceImpactPanel
                impact={selectedPieceImpact as any}
                    ...
                    <td colSpan={13} className="cmd-table__empty">
                      ...
                      <div>
                        {presentPieceControlError(
                          piecesError as Error,
                          "Piece Register data could not be loaded.",
                        )}
                      </div>
                      <button type="button" onClick={onRetryPieces} className="cmd-btn">Try again</button>
                    </td>
                  </tr>
                ) : null}
                {!piecesLoading && !piecesError && filteredRows.map((piece) => (
                  <tr key={piece.id}>
                    <td>
                      <input
                        type="checkbox"
                ...
                {!piecesLoading && !piecesError && filteredRows.length === 0 && (
                  <tr>
                    <td colSpan={13} className="cmd-table__empty">
                      ...
                      <button
                        type="button"
                        onClick={() => displayRows.length === 0 ? onGoImport() : clearRegisterFilters()}
