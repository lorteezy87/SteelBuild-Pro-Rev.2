...
export default function ProductionNotes() {
  ...
  // ─── Bullet row ─────────────────────────────────────────────────────────
  const BulletRow = ({ note, projectId, isLast, onCreateNext }) => {
    const [text, setText] = useState(note.content || "");
    const inputRef = useRef(null);

    // Keep local state in sync if the server replaces the optimistic row.
    useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      if (e.key === "Enter" && !e.shiftKey) {
        e.preventDefault();
        commit();
        // If this was the last bullet and has content, create a fresh empty bullet.
        if (isLast) {
          if (text.trim()) onCreateNext();
        } else {
          // Move focus to the next bullet input in the row.
        ...
        e.preventDefault();
        // Delete empty bullet on backspace, focus previous if available.
        const all = document.querySelectorAll(`[data-bullet-project="${projectId}"] [data-bullet-input]`);
        const idx = Array.from(all).indexOf(e.currentTarget);
        deleteBullet(note);
        setTimeout(() => {
        ...
        <div className="bullet-actions" style={{ display: "flex", gap: 4, opacity: 0.65, transition: "opacity 0.15s" }}>
          <button
            ...
            style={{
              ...
              background: highlighted ? "rgba(234,179,8,0.25)" : "transparent",
              color: highlighted ? "var(--status-warning)" : "var(--text-muted)",
              cursor: "pointer",
              display: "flex",
              alignItems: "center",
  ...
  const ProjectRow = ({ row }) => {
    ...
    // Always show at least one input slot so the row is writable.
    // If there are no bullets (only happens transiently on row creation),
    // we render a single empty bullet placeholder backed by an addBullet call.
        ...
        <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 2 }}>
          {renderable.map((note, idx) => (
            <BulletRow
              ...
              onCreateNext={() => addBullet(projectId, "")}
            />
          ))}
          <button
            onClick={() => addBullet(projectId, "")}
            style={{
