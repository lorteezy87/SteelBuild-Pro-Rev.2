...
export default function PackageBoard({
  ...
  const columns = useMemo(
    () =>
      buildPackageBoardColumns({
        // Snapshot rows are structural supersets of board input types.
        pieces: (snapshotQuery.data?.pieces ?? []) as BoardPiece[],
        workPackages: (snapshotQuery.data?.workPackages ?? []) as BoardWorkPackage[],
        markFilter,
        lifecycleFilter,
        assignmentOverrides,
      }),
