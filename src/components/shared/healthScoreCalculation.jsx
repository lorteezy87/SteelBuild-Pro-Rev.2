...
export async function calculateProjectHealthScore(projectId, base44) {
  ...
  try {
    ...
    const overdueAI = openAI.filter(a => a.due_date && new Date(a.due_date) < today);
    let aiScore = 100;
    if (openAI.length > 0)
    ...
    const lateDel = scheduledDel.filter(d => d.status !== "Delivered" && new Date(d.scheduled_date) < today);
    let delScore = 100;
