...
**Audit date:** 2026-09-21 · **Tree audited:** `ada5426d` (`main` == `claude/epic-ritchie-0chl93`, 0 commits apart at audit time; §9 reconciles against `main@7227d32`) · **Mode:** audit-only (no application code, migrations, configuration, dependencies, lockfiles or deployment settings were modified; this report is the only file added).
...
**Addendum (2026-09-22).** `main` advanced 35 commits past the audited tree before this report was opened as a PR; §9 reconciles every finding against `main@7227d32` and the production state re-read on 2026-09-22. Three high findings (AUTH-1, AUTH-3, CI-1) and eight medium ones are already closed there, and `sheets-api` is gone; the verdicts below are unchanged.
...
*(Superseded in part on 2026-09-22: `sheets-api` and `legacy-app-files-copy` have since been deleted from production and seven functions were redeployed; see §9.)*
...
## 9. Addendum (2026-09-22) — reconciliation against `main@7227d32`

The audit above was taken at `ada5426`. By the time the report was opened as a PR, `main` had advanced **35 commits** (PRs #460–#464: "complete Claude handoff fixes and restore staging", "launch security hardening", "restore titleblock action", "record sheets-api retirement", "email ingest build guard"). This branch has since merged `main` (`9fd7313`), and the following reconciliation was done by reading the diff `ada5426..7227d32` (130 files) and re-reading the production ledger and Edge Function inventory on 2026-09-22. Nothing here changes the method or evidence of §1–§9; it records which findings are already closed and which still stand.

**Why the branch's first CI run showed the drift check red.** The push-event run at 2026-09-21T07:38Z failed `Supabase drift check` with `unknownMigrations: ["20260921054458", "20260921055027"]` — two versions stamped in production that day whose files lived on the PR #460 branch, not on this one. That is exactly the "file lands with the stamp" hazard CLAUDE.md documents (DB-11). Both files are on `main` now, the pull-request run (which checks out the merge ref) passed, and merging `main` into this branch clears the push-run case too.

**Production state re-read on 2026-09-22 (management-plane, no SQL):** the ledger holds **130** versions (three new: `20260921054458`, `20260921055027`, `20260921080604`); the deployed function inventory is down to **9** — `sheets-api` and `legacy-app-files-copy` have been **deleted** (§2.5 is superseded on that point), and `llm-proxy` (v43), `stripe-billing` (v30), `project-export` (v30), `email-ingest` (v34), `email-send` (v30), `health` (v15) and both `command-center-*` functions were redeployed on 2026-09-21.

### 9.1 Findings closed or narrowed by `main` / production

| Finding | Status after `main@7227d32` | Evidence |
|---|---|---|
| AUTH-1 org admin can delete the sole owner | **Fixed in production** | `supabase/migrations/20260921080604_harden_workspace_membership_and_audit_boundaries.sql`: `enforce_org_member_guard` now `BEFORE INSERT OR UPDATE OR DELETE` with a last-owner check on DELETE and a `FOR UPDATE` lock on the org row; `org_members_delete` gains `(role <> 'owner' OR user_org_role_at_least(org_id,'owner'))`; version is in the ledger |
| AUTH-3 invite tokens readable by every member | **Fixed in production** (listing) | same migration: `org_invites_select` now `USING (user_org_role_at_least(org_id,'admin'))`; invitees still redeem through `get_invitation`/`accept_invitation`, whose bodies remain outside this repo |
| RLS-1 cross-tenant audit-row RPCs | **Fixed in production** | same migration: `log_backcharge_event` requires `pm` on the backcharge's project (or service role) and restores the GUC afterwards; `log_transmittal_event` derives the project from the transmittal, rejects a mismatched `p_project_id`, requires `pm` |
| RLS-2 viewer write floor missing on 13 tables | **Partially fixed in production** | same migration adds restrictive `field` INSERT/UPDATE/DELETE floors on `drawing_analyses`, `drawing_findings`, `external_linked_folders`, `email_accounts`, `email_messages`, `email_attachments`; its header states that production already carried PM-only sign-off/zone/link policies and field-only revision-delta policies (the repo baseline was stale for those — the drift caveat in §4.5 applies). Still membership-only: `activities` and `pma_audit_logs` INSERT (audit noise) |
| RLS-6 definer read oracles | **Mostly fixed in production** | `evaluate_fab_release_package`, `work_package_drawing_set_reports`, `piece_control_drawing_is_approved` now check `user_has_project_access` (or service role); `set_for_*_is_locked`, `users_share_org`, `founding_org_id` unchanged |
| RLS-8 `org_members_update` did not pin identity | **Fixed in production** | guard raises `Workspace membership identity cannot be changed` when `org_id`/`user_id` change |
| CI-1 secret scan / drift not deploy gates | **Fixed on main** | `.github/workflows/ci.yml`: `deploy-cloudflare` and `preview-cloudflare` now `needs: [ci, secret-scan, supabase-drift, edge-typecheck]` |
| CI-3 laptop deploy path | **Partially fixed on main** | `package.json` `deploy` now runs `scripts/require-ci-deploy.mjs`, which refuses with "Production publishing is restricted to the gated main GitHub Actions workflow"; branch protection is still an owner item |
| CI-4 `check:no-new-js` vacuous | **Fixed on main** | `ci` job checks out with `fetch-depth: 0`, passes `NO_NEW_JS_BASE`, and `scripts/check-no-new-js.mjs` now exits 1 when the base ref cannot be resolved |
| CI-6 staging gone / P0 E2E cannot run | **Partially fixed on main** | staging rebuilt on a persistent Supabase branch (`ndyfjffsulfbwpmwdmic`) and a separate Worker (`steelbuild-pro-staging`, `wrangler.staging.jsonc`, `supabase/config.staging.toml`); new `deploy-staging-cloudflare` job with an isolation check; both staging E2E jobs now depend on it. Mutation E2E remains opt-in (`STAGING_E2E_MUTATIONS_ENABLED`), so the P0 fab-gate spec still has no scheduled run |
| CI-8 no Edge Function typecheck | **Fixed on main** | new `edge-typecheck` job runs `deno check` over `supabase/functions/*/index.ts` and gates deploy/preview; a new `supabase-deploy-reviewed.yml` workflow adds a reviewed backend-release path (see below) |
| EDGE-1 `maxTokens` string bypass | **Fixed on main / v43** | `supabase/functions/llm-proxy/requestLimits.ts` `normalizeRequestLimits` coerces, rejects non-finite, floors and clamps `maxTokens` and bounds `temperature` before dispatch |
| EDGE-2 caller-trusted `useCase` / `project_id` | **Mostly fixed on main / v43** | `requestBoundary.ts` `authorizeTelemetryProject` verifies `project_id` with the caller's JWT via `user_has_project_access` (403 on denial, 503 if unverifiable); `quota.ts` no longer takes a `failClosed` flag — configured limits fail closed for every call, so `useCase` no longer selects the failure policy |
| EDGE-3 export audit row names a self-asserted name | **Fixed on main / v30** | `project-export` passes `performedByUserId: user.id` into `buildExportAuditRecord` (`exportShape.ts`) |
| EDGE-4 empty Stripe webhook secret accepted | **Fixed on main / v30** | `stripe-billing/configGuard.ts` `billingReadiness` returns 503 when the key or (on `/webhook`) the secret is empty; test-mode no longer falls back to the live key; `PRICE[plan]` guarded with `Object.hasOwn` |
| EDGE-13 unbounded body reads | **Partially fixed** | `llm-proxy` now streams with `readBoundedJson` (byte-counted, 413); `email-ingest`, `email-send`, `account-delete`, `project-export`, `stripe-billing` unchanged |
| EDGE-14 internal error text returned | **Partially fixed** | `stripe-billing` returns generic 400/500 text; `project-export/index.ts:331` still returns `Internal error: ${message}`; others unchanged |
| EDGE-16 `email_accounts` tokens in exports | **Fixed on main / v30** | `project-export/exportShape.ts:167-168` strips `access_token`/`refresh_token` from exported `email_accounts` rows; reads are now paged (`readTablePaged.ts`) |
| EDGE-8 `disabled.ts` not the deployed entrypoint | **Moot** | `legacy-app-files-copy` is no longer deployed |
| §2.5 `sheets-api` still live | **Superseded** | deleted from production; `production-ownership-manifest.json` marks it `deprecated` with the retirement record |
| PERF-11 `useDrawingRegister` unbounded view read | **Fixed on main** | `src/hooks/useDrawingRegister.ts` pages `drawing_register_view` with `fetchAllRows` and selects `drawing_set_id` directly; `ExportMarkupPDFModal` reads moved to a paged `src/lib/exports/markupExportData.ts`; both removed from the raw-`supabase.from()` grandfather list in `eslint.config.js` |
| DR-4 rollback runbook prescribes Vercel | **Fixed on main** | `docs/runbooks/rollback.md` now names the Cloudflare Worker version rollback for production and staging (`incident-response.md` also updated) |
| COMP-1 subprocessor list stale | **Partially fixed on main** | `src/pages/Subprocessors.jsx` now lists Cloudflare, Open-Meteo, Microsoft and Resend; all four legal pages still open with `// DRAFT — … MUST be reviewed by legal counsel` |
| TEST-2 no role-based RLS tests | **Partially fixed on main** | two staging-only, rolled-back psql suites were added: `supabase/tests/launch_security_boundaries.sql` (owner / admin / pm / viewer / field / outsider fixtures across three orgs, `set role` probes) and `supabase/tests/pending_issue_permissions.sql`; they are run by hand on the staging branch, not in CI |
| HYG-2 / §2.5 docs | **Partially fixed** | `supabase/README.md`, `docs/runbooks/staging-setup.md`, `supabase-production-ownership.md` updated; `README.md`/`ARCHITECTURE.md` claims noted in §4.11 unchanged |

**Correction to the audit.** §5's vocabulary row ("no UI-offered value a CHECK rejects was found") was wrong for RFIs: `chk_rfis_ball_in_court` rejects `"Engineer"`, which three writers still offered — the Detail Panel's ball-in-court buttons (`src/pages/rfis/constants.js`), the RFI log importer's `assigned_to || "Engineer"` fallback (`src/lib/importRfiLog.js`) and `EXTERNAL_BIC` in `rfiControlCenter.derive.ts` — so each lost the user's save with a raw constraint name. This was fixed on `main` by commit `d1c8389` (shared `BALL_IN_COURT_PARTIES` vocabulary, `normalizeBallInCourt` for imports, 13 new vocabulary tests) before the report landed; it is recorded here so the invariant table is not read as clean on that point.

**New observation.** The PR's check list includes three **Netlify** checks ("Header rules", "Redirect rules", "Pages changed — steelbuild-pro"), so the old Netlify site is still connected to the repository and builds every pull request. `public/_headers` acknowledges this ("while the Netlify site is still connected"). Confirm that site cannot publish to the production hostnames and disconnect it; an extra publisher outside the gated workflow is the same class of risk as the Workers Builds integration (CI-10).

### 9.2 Findings that still stand after `main@7227d32`

No file touched by these findings changed between `ada5426` and `7227d32` unless noted.

- **Authentication/session:** AUTH-2 (client-only, racy MFA gate), AUTH-4 (recovery session reaches the app), AUTH-5 (implicit flow; `src/lib/supabase.ts` unchanged), AUTH-6 (logout leaves tenant caches), AUTH-7 (consent-free desktop hand-off), AUTH-8 (no re-authentication), AUTH-9, AUTH-10, AUTH-11.
- **Edge Functions:** EDGE-5 (self-deletion fails for sole owners with live projects; `account-delete` unchanged, still v3), EDGE-6, EDGE-7 (`account-delete` and `email-ingest` still hardcode `Access-Control-Allow-Origin: *`), EDGE-9, EDGE-10, EDGE-11, EDGE-12, EDGE-15.
- **Client data layer:** DATA-1 (pay-app contract key vs the global `['projects']` select default), DATA-2 (schedule bulk toolbar), DATA-3 (`Number(null)` percent), DATA-4 (delivery importers), DATA-5 (CO import dedup), DATA-6, DATA-7, DATA-8 (`Constraints.jsx:145-146` still `.catch(() => [])`), DATA-9 … DATA-15.
- **Business rules:** LOGIC-1 (ZonePanel invents RFI numbers) through LOGIC-12 — none of `ZonePanel.jsx`, `marginRiskEngine.ts`, `submittalWorkdayDue.ts`, `pccEngine.ts`, `submittalStageMapping.ts`, `ofsCompletionGate.ts` changed.
- **RLS:** RLS-3 (plan limit only in RPC), RLS-4 (`number_sequences` writable below the RPC), RLS-5 (org-scoped storage), RLS-7 (`feature_flags` overrides readable), RLS-9 (default privileges), the `activities`/`pma_audit_logs` remainder of RLS-2.
...
### 9.3 Revised headline after `main@7227d32`
Of the 20 high findings, **3 are closed** (AUTH-1, AUTH-3, CI-1) and CI-3 is narrowed; of the 54 medium findings, **8 are closed** (RLS-1, RLS-8, CI-4, CI-8, EDGE-1, EDGE-3, EDGE-4, DR-4) and RLS-2, RLS-6, EDGE-2, EDGE-13, EDGE-14, EDGE-16, CI-6, COMP-1 are narrowed. The P0 list in §7 therefore shrinks to: AUTH-2/4/5/6 (session hardening), CI-2 (environment-scoped deploy secrets) plus branch protection, OBS-1 / DR-1 / DR-2 (owner), DATA-1/2/3/4/5 and LOGIC-1/5/6 (invariant regressions), RLS-2's remainder, and MOB-1 … MOB-5 before any store submission. The verdicts in the Executive verdict table are unchanged: the web app is close, the iOS shell is not submittable yet, Android does not exist.

## 10. Appendix — authenticated-callable SECURITY DEFINER RPCs (live advisor, 2026-09-21)
