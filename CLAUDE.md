...
- CI gates — every PR must pass all of them: `lint`, `typecheck`, `typecheck:js`, `typecheck:strict`, `typecheck:noimplicitany`, `check:no-new-js`, `test`, `build`.
  ...
  - **A deploy is gated on four jobs, not just `ci`.** `deploy-cloudflare` declares `needs: [ci, secret-scan, supabase-drift, edge-typecheck]`; `preview-cloudflare` needs the same with `supabase-drift-branch`, because `supabase-drift` runs on `main` only. `edge-typecheck` is a Deno check over the released Edge Functions, so an Edge Function that does not type-check blocks the web deploy too.
  - **Credential-holding jobs name an environment** (`production`, `preview`, `staging`) — a job's `if:` is written by whatever branch pushed the file, so it cannot scope a secret. `scripts/__tests__/ciSecretScope.test.ts` enforces it; the owner still has to move the secrets in (`docs/runbooks/owner-checklist.md` §7).
...
- **Commit the file in the same change that applies the migration — file first if anything.** `Supabase drift check` reads production's ledger against the repo's `supabase/migrations/` on **every branch** (off `main` as `Supabase drift check (branch)`, which runs main's checker over the branch's files), so a stamp whose file is still unmerged turns `main` red *and* every open PR red, each one a PR whose own diff is innocent. Once `SUPABASE_ACCESS_TOKEN` is main-only, branches fall back to a manifest-only check with a warning and only `main` (plus `supabase-drift-scheduled.yml`) sees the ledger — so an unapplied migration no longer shows on its PR; it turns `main` red at merge. This has happened three times in two days: `20260919082758` (#441), `20260920014500` (#445), `20260921034212` (#455). Each cost several sessions a red-CI diagnosis.
