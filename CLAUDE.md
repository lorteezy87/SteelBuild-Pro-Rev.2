...
- **Clients never write `number_sequences`** (RLS-4, `supabase/migrations/*_lock_number_sequences_writes.sql` — written 2026-09-23, not yet applied). It drops the member INSERT/UPDATE/DELETE policies and the table write grants, because a field user could `PATCH next_value = 1` and make the RPC re-issue existing numbers. Only SECURITY DEFINER functions write it. Never add a client write policy back; an admin "reset counter" needs its own definer RPC with a role check.
