...
import ScheduleImportStaging from "@/components/schedule/ScheduleImportStaging";
...
import { downloadMSProjectXML } from "@/lib/integrations/msProjectExport";
import { downloadCSV, downloadXLSX } from "@/lib/integrations/scheduleExport";
import { openFieldLookahead } from "@/lib/integrations/fieldLookahead";
...
export default function Schedule() {
  ...
  const [showImportStaging, setShowImportStaging] = useState(false);
  ...
  const handleImportMPP = async (file) => {
    ...
    if (fileName.endsWith('.mpp') && !fileName.endsWith('.xml')) {
      toast.error("Binary .mpp files are not supported. Please export from MS Project as XML first (File > Save As > XML).");
    ...
    try {
      ...
      if (!allParsed.length) {
        throw new Error("No tasks found. Please export the MPP as XML and retry.");
      }
      // Stage tasks for preview - don't create anything yet
      setStagedTasks(allParsed);
      setStagedFileName(file.name);
      setShowImportStaging(true);
    } catch (e) {
  ...
  /**
   * Confirm import: creates all staged tasks in the database. Called from
   * ScheduleImportStaging after the user reviews and clicks "Confirm Import".
   */
  const confirmStagedImport = async (allParsed) => {
    if (!projectId) return;
    setImporting(true);
    try {
      const pid = projectId;
      ...
      invalidateEntity(qc, "schedule_task", projectId);
      setShowImportStaging(false);
      setStagedTasks([]);
      toast.success(`Imported ${Object.keys(uidToDbId).length} tasks from ${stagedFileName}`);
    } catch (e) {
      toast.error(e.message || "Import failed");
    } finally {
      setImporting(false);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        >
          ...
          <div style={{ position: "relative" }}>
            <Button
              ...
              title="Export schedule to MS Project, CSV, XLSX, or field look-ahead"
            >
              EXPORT
            </Button>
            {showExportMenu && (
              ...
              >
                <button
                  onClick={() => {
                    const filename = downloadMSProjectXML(tasksWithEffective, {
                ...
                >
                  <span style={exportMenuIconStyle}>XML</span>
                ...
                <button
                  onClick={() => {
                    const filtered = phaseFilter === "all"
                      ? tasksWithEffective
                      : tasksWithEffective.filter((t) => t.phase === phaseFilter);
                    const filename = downloadCSV(filtered, tasksWithEffective, selectedProject?.project_number || "project");
                    toast.success(`Exported ${filename} (${filtered.length} tasks)`);
                    setShowExportMenu(false);
                  }}
                  style={exportMenuItemStyle}
                >
                  <span style={exportMenuIconStyle}>CSV</span>
                  Export CSV
                </button>
                <button
                  onClick={() => {
                    const filtered = phaseFilter === "all"
                      ? tasksWithEffective
                      : tasksWithEffective.filter((t) => t.phase === phaseFilter);
                    const filename = downloadXLSX(filtered, tasksWithEffective, selectedProject?.project_number || "project");
                    toast.success(`Exported ${filename} (${filtered.length} tasks)`);
                    setShowExportMenu(false);
                  }}
                  style={exportMenuItemStyle}
                >
                  <span style={exportMenuIconStyle}>XLS</span>
                  Export XLSX
                </button>
                <div style={{ height: 1, background: "var(--divider)", margin: "4px 8px" }} />
                <button
                  onClick={() => {
                    const count = openFieldLookahead({
                      tasks: tasksWithEffective,
                      allTasks: tasksWithEffective,
                      weeks: 3,
                      project: selectedProject || {},
                    });
                    toast.success(`3-week look-ahead: ${count} tasks`);
                    setShowExportMenu(false);
                  }}
                  style={exportMenuItemStyle}
                >
                  <span style={exportMenuIconStyle}>3W</span>
                  3-Week Field Look-Ahead
                </button>
                <button
                  onClick={() => {
                    const count = openFieldLookahead({
                    ...
                    toast.success(`4-week look-ahead: ${count} tasks`);
                    setShowExportMenu(false);
                  }}
                  style={exportMenuItemStyle}
                >
                  <span style={exportMenuIconStyle}>4W</span>
                  4-Week Field Look-Ahead
                </button>
              </div>
            )}
          </div>
      ...
      {/* Schedule Import Staging Modal */}
      <ScheduleImportStaging
        open={showImportStaging}
        onClose={() => { setShowImportStaging(false); setStagedTasks([]); }}
        onConfirm={confirmStagedImport}
        parsedTasks={stagedTasks}
        ...
        sourceFileName={stagedFileName}
      />
    </div>
  );
}

// ── Export menu item style ────────────────────────────────────────────────
