...
const SOFT_DELETE_TABLES = new Set<string>([
  ...
  // Document storage integration — external file refs track soft-delete
  'external_file_refs',
  // BIM Integration: model registry supports soft-delete
  'model_registry',
]);
...
export const entities = {
  ...
  // Document Storage Integration: external provider linking
  ExternalFileRef:       createEntityClient('external_file_refs'),
  ExternalLinkedFolder:  createEntityClient('external_linked_folders'),
  // Autodesk / IFC / BIM Integration: model registry and element links
  ModelRegistry:         createEntityClient('model_registry'),
  ModelElementLink:      createEntityClient('model_element_links'),
  // Email Integration: intake queue and per-project settings
  EmailIntakeQueue:      createEntityClient('email_intake_queue'),
  EmailIntegrationSetting: createEntityClient('email_integration_settings'),
};
