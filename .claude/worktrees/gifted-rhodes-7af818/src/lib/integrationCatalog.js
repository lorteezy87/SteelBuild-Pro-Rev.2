export const INTEGRATION_AREAS = [
  ...
  },
  {
    ...
    existingCapabilities: [
      ...
      "PDF Import Review Queue stages all imports for human approval before record creation.",
      "Markup Export packages selected drawings into consolidated issue-review PDFs.",
      "Source-file lineage tracked on every imported record (file name, hash, confidence).",
    ],
    targetWorkflows: [
    ...
    nextSprint: [
      "Add Bluebeam Studio session sync for real-time collaborative markup.",
      "Implement revision-comparison diff view between drawing versions.",
      "Extend markup export with embedded thumbnails from PDF pages.",
    ],
  },
  {
    key: "scheduling",
  ...
  },
  {
    ...
    status: "Live",
    risk: "Medium",
    shortDescription: "Model registry with IFC/GLTF/GLB file tracking, element-to-entity linking, and Autodesk cloud references.",
    systems: ["IFC", "Revit exports", "Autodesk Construction Cloud", "Autodesk Platform Services"],
    existingCapabilities: [
      "Model Viewer loads GLTF, GLB, and IFC files locally with @thatopen/components.",
      "Model Registry tracks models by project with version, revision, and coordinate-system metadata.",
      "Element-level linking connects IFC elements to RFIs, Drawings, and Work Packages.",
      "Autodesk Construction Cloud references stored without OAuth (external link with model ID).",
      "Documents can store IFC Model category files and be registered as models.",
    ],
    targetWorkflows: [
      "Model file registry by project and revision — LIVE.",
      "Element-to-RFI/drawing/work-package linking — LIVE.",
      "Autodesk cloud model reference without uncontrolled file copying — LIVE.",
      "Autodesk OAuth for direct model sync (deferred until registry is field-proven).",
    ],
    dataTouched: ["Documents", "Drawings", "RFIs", "Work Packages", "Model Registry", "Model Element Links"],
    prerequisites: ["Autodesk app registration (deferred)", "Model version ids", "Element id strategy", "Permission boundaries"],
    nextSprint: [
      "Field-test model registry with real IFC uploads from active projects.",
      "Add bulk element linking from Model Viewer click-to-select.",
      "Evaluate Autodesk Platform Services OAuth scope once registry usage stabilizes.",
    ],
  },
];

export const INTEGRATION_STATUSES = ["All", "Live", "Partially Live", "Planned", "Adapter Required"];
