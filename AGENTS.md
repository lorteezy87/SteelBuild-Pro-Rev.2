...
Use Conventional Commit prefixes seen in history: `feat:`, `fix:`, `docs:`, `refactor:`, `ui:`, `security:`. CI (`.github/workflows/ci.yml`) runs every gate listed under `CLAUDE.md` → Commands on every push/PR. **Only a green `ci` job lets the gated deploy job publish to Cloudflare Workers** (Worker `steelbuild-pro-rev-2`; see `CLAUDE.md` → Deploy). Vercel is retired. Feature work lands on `claude/*` branches or directly on `main`.
...
- **Staging is down:** the Vercel staging project and the staging Supabase project are both gone (see the header of `.github/workflows/ci.yml`). The staging E2E jobs can't run until staging is rebuilt (`docs/runbooks/staging-setup.md`). If GitHub Actions jobs fail at start, check the org billing/spending limit.
