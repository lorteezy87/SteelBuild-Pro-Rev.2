...
- **Staging EnvValidationError:** If staging throws `VITE_SUPABASE_URL must be an http(s) URL`, the staging Vercel env has the anon key in the URL slot — fix in Vercel project env (not app code). GitHub Actions CI currently fails at start when the org billing/spending limit blocks runners.
