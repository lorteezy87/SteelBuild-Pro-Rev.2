-- Forward repair of four independently verified missing effects, 2026-09-13.
-- No migration-ledger edits and no change_requests/other policy changes.
-- Audited readiness prosrc MD5: 36452c1b163ba3065fe2741e95f676e0 (production).
-- Desired prosrc MD5: faeeed62ac7dc3946b971c19908ed3bd (canonical 20260802010000).
-- Body reproduced exactly; all preconditions run before any semantic mutation.
-- Role floors accept only the audited field or desired pm policy shapes.
-- The migration runner owns the transaction, including the ledger write.
...
BEGIN
  ...
  IF NOT FOUND OR pg_catalog.md5(v_before.prosrc) NOT IN ('36452c1b163ba3065fe2741e95f676e0','faeeed62ac7dc3946b971c19908ed3bd')
  ...
  FOREACH v_table IN ARRAY ARRAY['drawing_impacts','email_integration_settings'] LOOP
    ...
    FOREACH v_suffix IN ARRAY ARRAY['ins','upd','del'] LOOP
      ...
      IF (((v_command='a' AND v_using IS NULL AND v_check=v_field) OR (v_command='w' AND v_using=v_field AND v_check=v_field) OR (v_command='d' AND v_using=v_field AND v_check IS NULL)) OR ((v_command='a' AND v_using IS NULL AND v_check=v_pm) OR (v_command='w' AND v_using=v_pm AND v_check=v_pm) OR (v_command='d' AND v_using=v_pm AND v_check IS NULL))) IS NOT TRUE THEN
  ...
  IF NOT FOUND OR v_column.atttypid IS DISTINCT FROM 'text'::regtype
     ...
     OR (pg_catalog.col_description(v_column.attrelid,v_column.attnum) IS NULL OR pg_catalog.col_description(v_column.attrelid,v_column.attnum)='Free-text materials received that day (field superintendent daily log).') IS NOT TRUE THEN
...
$readiness_definition$;
  ...
  FOREACH v_table IN ARRAY ARRAY['drawing_impacts','email_integration_settings'] LOOP
    EXECUTE pg_catalog.format('ALTER POLICY %I ON public.%I WITH CHECK (public.user_has_project_role_at_least(project_id, %L))',v_table||'_ins_role_floor',v_table,'pm');
    EXECUTE pg_catalog.format('ALTER POLICY %I ON public.%I USING (public.user_has_project_role_at_least(project_id, %L)) WITH CHECK (public.user_has_project_role_at_least(project_id, %L))',v_table||'_upd_role_floor',v_table,'pm','pm');
    EXECUTE pg_catalog.format('ALTER POLICY %I ON public.%I USING (public.user_has_project_role_at_least(project_id, %L))',v_table||'_del_role_floor',v_table,'pm');
  END LOOP;
  COMMENT ON COLUMN public.daily_logs.materials_received IS 'Free-text materials received that day (field superintendent daily log).';
