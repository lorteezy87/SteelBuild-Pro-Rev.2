...
BEGIN
  ...
  resolved AS (
    SELECT
      ...
      e.old_piece_id,
      CASE
        WHEN e.lot IS NOT NULL THEN uml.piece_id
        ELSE um.piece_id
      END AS piece_id,
      CASE
        WHEN e.lot IS NOT NULL THEN uml.work_package_id
        ELSE um.work_package_id
      END AS work_package_id,
      CASE
        WHEN e.lot IS NOT NULL THEN uml.lifecycle_status
        ELSE um.lifecycle_status
    ...
    LEFT JOIN uniq_mark_lot uml
      ON e.lot IS NOT NULL
     AND uml.mark = e.mark
     AND uml.lot = e.lot
    LEFT JOIN uniq_mark um
      ON e.lot IS NULL
     AND um.mark = e.mark
    LEFT JOIN amb_mark_lot aml
      ON e.lot IS NOT NULL
     AND aml.mark = e.mark
     AND aml.lot = e.lot
    LEFT JOIN amb_mark am
      ON e.lot IS NULL
     AND am.mark = e.mark
  ),
  _applied AS (
    ...
    SET
      ...
      fab_status = r.lifecycle_status,
      updated_at = now()
    FROM resolved r
    WHERE me.id = r.element_id
      AND r.match_count = 1
      AND (
        me.piece_id IS DISTINCT FROM r.piece_id
        OR me.work_package_id IS DISTINCT FROM r.work_package_id
        OR me.fab_status IS DISTINCT FROM r.lifecycle_status
      )
    RETURNING me.id
  ),
  counts AS (
    SELECT
      ...
      count(*) FILTER (
        WHERE r.match_count = 1
          AND r.old_piece_id IS NOT DISTINCT FROM r.piece_id
      )::integer AS unchanged,
      count(*) FILTER (
        WHERE r.match_count = 1
          AND r.old_piece_id IS DISTINCT FROM r.piece_id
      )::integer AS linked,
...
-- Full-project entry point: still available, but optimized + higher timeout.
-- Prefer the page RPC from the app for large rosters.
